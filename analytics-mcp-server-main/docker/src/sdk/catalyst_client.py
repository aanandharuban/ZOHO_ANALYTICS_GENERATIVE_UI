...
class CatalystCache:
    ...
    def submit_request(
        self, method: str, url: str, data: Any = None, params: Dict = None
    ) -> ResponseObject:
        """
        ...
        try:
            if method == "POST" and isinstance(data, str):
                ...
                headers = self._get_headers(
                    "application/json" if data and method in ["POST", "PUT"] else None
                )
    ...
    def _execute_with_retry(
        ...
        if not str(resp_obj.status_code).startswith("2") and self.is_oauth_expired(
            resp_obj
        ):
    ...
    def insert(
        self, cache_name: str, cache_value: str, expiry_in_hours: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        Raises:
            ServerError: If request fails
        """
        payload = {"cache_name": cache_name, "cache_value": cache_value}
