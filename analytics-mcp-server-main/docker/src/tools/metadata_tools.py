...
async def get_workspaces_list(
    include_shared_workspaces: bool, contains_str: str | None = None
) -> list[dict] | str:
    """
    <use_case>
    ...
    try:
        ...
        if not include_shared_workspaces:
            ...
            return filter_and_limit_workspaces(
                workspaces, contains_str, owned_flag=True, limit=WORKSPACE_RESULT_LIMIT
            )
        else:
            workspaces = await asyncio.to_thread(analytics_client.get_workspaces)
            owned_result = filter_and_limit_workspaces(
                workspaces.get("ownedWorkspaces", []),
                contains_str,
                owned_flag=True,
            ...
            shared_result = filter_and_limit_workspaces(
                workspaces.get("sharedWorkspaces", []),
                contains_str,
                owned_flag=False,
...
async def get_view_details(view_id: str) -> dict:
    ...
    try:
        analytics_client = get_analytics_client_instance()
        view_details = await asyncio.to_thread(
            analytics_client.get_view_details,
            view_id,
...
async def search_views(
    ...
    try:
        ...
        if (view_contains_str is not None and view_contains_str.strip() != "") or (
            natural_language_query is None or natural_language_query.strip() == ""
        ):
            ...
            view_list = await retry_with_fallback(
                [org_id],
                workspace_id,
                "WORKSPACE",
                get_views,
            ...
            for view in view_list:
                filtered_view = {
                    ...
                    if k in {"viewId", "viewName", "viewDesc"}
                }
            ...
            ctx = get_context()
            while (
                len(current_view_list) > 15 and epoch <= max_epochs and sample_supported
            ):
                await ctx.info(
                    f"Starting Epoch {epoch} with {len(current_view_list)} views"
                )
                ...
                for batch_number in range(number_of_batches):
                    views_in_batch = current_view_list[
                    ...
                    try:
                        ...
                        if batch_number == 0 and epoch == 1:
                            await ctx.info(
                                "Sampling is not supported in this environment"
                            )
                ...
                await ctx.info(
                    f"Epoch {epoch} completed. Reduced from {len(current_view_list)} to {len(filtered_view_list)} views"
                )
            ...
            if not sample_supported:
                await ctx.info(
                    "Using fallback mechanism: Returning first 20 views from the workspace"
                )
                return transformed_view_list[:20]

            await ctx.info(
                f"Final result: {len(current_view_list)} views after {epoch - 1} epochs"
            )
