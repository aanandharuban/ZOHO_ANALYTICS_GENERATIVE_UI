...
class RedisTokenBucketRateLimiter(RateLimiter):
    ...
    async def allow_tokens(self, key: str, tokens: int = 1) -> bool:
        ...
        allowed = await self.script(
            keys=[key], args=[self.capacity, self.refill_rate, tokens]
        )
...
async def rate_limiter_cleanup_task(
    limiter: InMemoryTokenBucketRateLimiter, interval_seconds: int = 60
):
    logger.info(
        f"Starting rate limiter cleanup task with interval {interval_seconds} seconds."
    )
    try:
        while True:
