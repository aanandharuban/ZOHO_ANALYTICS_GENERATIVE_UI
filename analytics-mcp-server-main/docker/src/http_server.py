...
from src.auth.remote_auth import (
    registed_clients_store,
    auth_transactions_store,
    auth_codes_store,
    client_ip_vs_client_ids_store,
)
...
async def lifespan(app: FastAPI):
    ...
    app.state.global_rate_limiter = await build_rate_limiter(
        capacity=Settings.GLOBAL_OAUTH_RATE_LIMIT_CAPACITY,
        window_seconds=Settings.GLOBAL_OAUTH_RATE_LIMIT_WINDOW,
    )

    for limiter in _rate_limiter_cache.values():
        if isinstance(limiter, InMemoryTokenBucketRateLimiter):
            background_tasks.append(
                asyncio.create_task(rate_limiter_cleanup_task(limiter))
            )

    if background_tasks:
        logger.info(
            f"Started {len(background_tasks)} total background cleanup task(s)."
        )
