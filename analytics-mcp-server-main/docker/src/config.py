...
class Settings:
    ...
    ## Tools
    ANALYTICS_WORKSPACE_LIST_RESULT_SIZE = int(
        os.getenv("ANALYTICS_WORKSPACE_LIST_RESULT_SIZE") or 20
    )
    ANALYTICS_VIEW_LIST_RESULT_SIZE = int(
        os.getenv("ANALYTICS_VIEW_LIST_RESULT_SIZE") or 15
    )
    ...
    QUERY_DATA_QUERY_EXECUTION_TIMEOUT = int(
        os.getenv("QUERY_DATA_QUERY_EXECUTION_TIMEOUT") or 30
    )
    ...
    TRUSTED_PROXY_LIST: list[IPv4Network | IPv6Network] = (
        [
            ...
            for ip in os.getenv("TRUSTED_PROXY_LIST", "").split(",")
            if ip.strip()
        ]
        if BEHIND_PROXY
        else []
    )
    DEPLOYMENT_SCENARIO: Literal["private_network", "public_network"] = os.getenv(
    ...
    CLIENT_IP_HEADER: str = (
        None if os.getenv("CLIENT_IP_HEADER") is None else os.getenv("CLIENT_IP_HEADER")
    )
    ...
    CATALYST_SDK_APP_NAME = os.getenv(
        "CATALYST_SDK_APP_NAME", "ZohoAnalyticsRemoteMCPServer"
    )
    ...
    CATALYST_PROJECT_DOMAIN = os.getenv(
        "CATALYST_PROJECT_DOMAIN", "https://api.catalyst.zoho.in"
    )

    CONSTANT_REMOTE_HOSTED_LOCATION = "REMOTE"
    CONSTANT_LOCAL_HOSTED_LOCATION = "LOCAL"

    OAUTH_DEFAULT_SCOPE = os.getenv(
        "OAUTH_DEFAULT_SCOPE", "ZohoAnalytics.fullaccess.all"
    )
    OAUTH_OFFLINE_ACCESS_SCOPE = os.getenv(
        "OAUTH_OFFLINE_ACCESS_SCOPE", "offline_access"
    )
    ...
    OAUTH_REGISTERED_CLIENTS_TTL = int(
        os.getenv("OAUTH_REGISTERED_CLIENTS_TTL", "36000")
    )
    OAUTH_CLIENT_IP_MAPPING_TTL = int(os.getenv("OAUTH_CLIENT_IP_MAPPING_TTL", "18000"))

    GLOBAL_OAUTH_RATE_LIMIT_CAPACITY = int(
        os.getenv("GLOBAL_OAUTH_RATE_LIMIT_CAPACITY", "30")
    )
    GLOBAL_OAUTH_RATE_LIMIT_WINDOW = int(
        os.getenv("GLOBAL_OAUTH_RATE_LIMIT_WINDOW", "60")
    )

    PRIVATE_OAUTH_STANDARD_RATE_LIMIT_COUNT = int(
        os.getenv("PRIVATE_OAUTH_STANDARD_RATE_LIMIT_COUNT", "5")
    )
    PRIVATE_OAUTH_STANDARD_RATE_LIMIT_WINDOW = int(
        os.getenv("PRIVATE_OAUTH_STANDARD_RATE_LIMIT_WINDOW", "60")
    )
    PUBLIC_OAUTH_STANDARD_RATE_LIMIT_COUNT = int(
        os.getenv("PUBLIC_OAUTH_STANDARD_RATE_LIMIT_COUNT", "100")
    )
    PUBLIC_OAUTH_STANDARD_RATE_LIMIT_WINDOW = int(
        os.getenv("PUBLIC_OAUTH_STANDARD_RATE_LIMIT_WINDOW", "60")
    )

    PRIVATE_OAUTH_REGISTRATION_RATE_LIMIT_COUNT = int(
        os.getenv("PRIVATE_OAUTH_REGISTRATION_RATE_LIMIT_COUNT", "10")
    )
    PRIVATE_OAUTH_REGISTRATION_RATE_LIMIT_WINDOW = int(
        os.getenv("PRIVATE_OAUTH_REGISTRATION_RATE_LIMIT_WINDOW", "3600")
    )
    PUBLIC_OAUTH_REGISTRATION_RATE_LIMIT_COUNT = int(
        os.getenv("PUBLIC_OAUTH_REGISTRATION_RATE_LIMIT_COUNT", "50")
    )
    PUBLIC_OAUTH_REGISTRATION_RATE_LIMIT_WINDOW = int(
        os.getenv("PUBLIC_OAUTH_REGISTRATION_RATE_LIMIT_WINDOW", "3600")
    )

    PRIVATE_OAUTH_MAX_CLIENTS_PER_IP = int(
        os.getenv("PRIVATE_OAUTH_MAX_CLIENTS_PER_IP", "5")
    )
    PUBLIC_OAUTH_MAX_CLIENTS_PER_IP = int(
        os.getenv("PUBLIC_OAUTH_MAX_CLIENTS_PER_IP", "0")
    )
    ...
    def get_allowed_org_ids():
        ...
        return [
            org_id.strip()
            for org_id in Settings.MCP_SERVER_ORG_IDS.split(",")
            if org_id.strip()
        ]
...
def get_analytics_client_instance(access_token=None) -> AnalyticsClient:
    ...
    if not analytics_client:
        analytics_client = AnalyticsClient.from_refresh_token(
            Settings.CLIENT_ID, Settings.CLIENT_SECRET, Settings.REFRESH_TOKEN
        )
        if (
            Settings.accounts_server_url() is None
            or Settings.ANALYTICS_SERVER_URL is None
        ):
            raise RuntimeError(
                "ACCOUNTS_SERVER_URL (or) ANALYTICS_SERVER_URL environment variable is not set. Please set it to your Zoho Analytics accounts server URL and analytics server URL respectively."
            )
